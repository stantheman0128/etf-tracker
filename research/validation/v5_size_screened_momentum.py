"""Strategy B: Size-screened momentum on PIT SP500.

Two-step: (1) keep only top 30% by price (cap proxy)
          (2) rank survivors by 12M momentum, pick top N

This is "quality + momentum" lite — using size as quality proxy filters out
the small-cap zombie stocks that pure momentum buys before they crash.
"""
...
# ─── KEY: two-step ranking ──────────────────────────────────────────────────
SIZE_PCTILE = 0.30  # keep top 30% by price (cap proxy)
...
def build_strategy(n, lookback, rebal_days, select_random=False, seed=None):
    ...
    for i in range(len(ALL_DATES)):
        ...
        if i % rebal_days == 0:
            pit = universe_at(d)
            # Step 1: size filter — keep top 30% by price
            with_px = [(t, price_at(t, i)) for t in pit]
            with_px = [(t, p) for t, p in with_px if p is not None and p > 0]
            if len(with_px) < n:
                out.append((d, last_pv)); continue
            with_px.sort(key=lambda x: -x[1])
            k_size = max(n, int(len(with_px) * SIZE_PCTILE))
            size_pool = [t for t, _ in with_px[:k_size]]
...
print("=" * 80)
print(f"STRATEGY B: Size-screened momentum (top {int(SIZE_PCTILE*100)}% by cap, then momentum Top N)")
print("=" * 80)
...
PERMS = int(os.environ.get("V5_PERMS", "1000"))
...
for vi, (n, lb, rb, label) in enumerate(BONF):
    ...
    perms = [stats(build_strategy(n, lb, rb, select_random=True, seed=90000+vi*PERMS+s))[0] for s in range(PERMS)]
    perms.sort()
...
print("\n" + "=" * 80)
print("STRATEGY B FINAL SCORECARD")
print("=" * 80)
...
out = {
    "strategy": "B_size_screened_momentum_pit",
...
(HERE / "v5_results.json").write_text(json.dumps(out, indent=2), encoding="utf-8")
print(f"\nResults -> {HERE / 'v5_results.json'}")
