"""Strategy A: Market-cap Top X on PIT SP500 (1996-2026).

Uses share-price as proxy for market cap (correlation ~0.7 within SP500).
Same PIT universe + same 4-gate framework as v3, only ranking differs.
"""
...
# ─── KEY DIFFERENCE: rank by PRICE (mkt cap proxy), not momentum ────────────
    ...
    for i in range(len(ALL_DATES)):
        ...
        if i % rebal_days == 0:
            ...
            for t in pit:
                ...
                elig.append((t, px, px))  # rank-key = price (cap proxy)
            if not elig:
...
print("=" * 80)
print("STRATEGY A: Market-cap Top X on PIT SP500")
print("=" * 80)

# Canonical: Top 3 monthly
t0 = time.time()
...
for s, e, lab in windows:
    ...
    if s_stats and spy_st:
        ...
        wf_results.append({"window":lab,"strat":s_stats[0],"spy":spy_st[0],"spread":spread,"strat_dd":s_stats[2],"spy_dd":spy_st[2]})
...
PERMS = int(os.environ.get("V4_PERMS", "1000"))
...
for vi, (n, rb, label) in enumerate(BONF):
    ...
    perms = [stats(build_strategy(n, rb, select_random=True, seed=80000+vi*PERMS+s))[0] for s in range(PERMS)]
    perms.sort()
...
# Gate 4 Bootstrap
print(f"\nBootstrap CI (1000 sims, 21-day blocks):")
...
# Final
print("\n" + "=" * 80)
print("STRATEGY A FINAL SCORECARD")
print("=" * 80)
...
out = {
    "strategy": "A_market_cap_top_x_pit",
...
(HERE / "v4_results.json").write_text(json.dumps(out, indent=2), encoding="utf-8")
print(f"\nResults -> {HERE / 'v4_results.json'}")
