"""Strategy v6: Size + Low-vol + Momentum on PIT SP500.

Three-layer filter (MTUM ETF style):
1. Size filter: PIT SP500 ∩ top 30% by cap (excludes mid-cap zombies)
2. Low-vol filter: from step 1, keep bottom 50% by 63-day volatility (excludes whipsaw stocks)
3. Momentum ranking: pick top N by 12M momentum within survivors

Hypothesis: layered quality (size) + risk control (low-vol) before momentum should give
higher Sharpe than v5b at the cost of slightly lower CAGR.
"""
...
def volatility(t, idx, window=63):
    """Annualized stdev of daily log returns over the window ending at idx."""
    if t not in PX_SORTED_IDX: return None
    rets = []
    prev = None
    for j in range(idx - window, idx):
        ...
        prev = p
    if len(rets) < window // 2: return None
...
def build_strategy(n, lookback, rebal_days, select_random=False, seed=None):
    ...
    for i in range(len(ALL_DATES)):
        ...
        if i % rebal_days == 0:
            pit = universe_at(d)
            # Layer 1: Size filter — top 30% by cap
            ...
            # Layer 2: Low-vol filter — bottom 50% by 63-day vol
            with_vol = [(t, volatility(t, i)) for t in size_pool]
            with_vol = [(t, v) for t, v in with_vol if v is not None and v > 0]
            if len(with_vol) < n:
                out.append((d, last_pv)); continue
            with_vol.sort(key=lambda x: x[1])  # ascending vol
            k_vol = max(n, int(len(with_vol) * LOWVOL_PCTILE))
            lowvol_pool = [t for t, _ in with_vol[:k_vol]]
            # Layer 3: Momentum ranking
            elig = []
            for t in lowvol_pool:
                px = price_at(t, i)
...
print("=" * 80)
print(f"STRATEGY v6: Size (top {int(SIZE_PCTILE*100)}%) -> Low-vol (bottom {int(LOWVOL_PCTILE*100)}%) -> Momentum Top N")
print("=" * 80)
...
PERMS = int(os.environ.get("V6_PERMS", "1000"))
...
for vi, (n, lb, rb, label) in enumerate(BONF):
    ...
    perms = [stats(build_strategy(n, lb, rb, select_random=True, seed=160000+vi*PERMS+s))[0] for s in range(PERMS)]
    perms.sort()
...
print("\n" + "=" * 80)
print("STRATEGY v6 FINAL SCORECARD")
print("=" * 80)
...
out = {
    "strategy": "v6_size_lowvol_momentum_pit",
    "size_pctile": SIZE_PCTILE, "lowvol_pctile": LOWVOL_PCTILE,
...
(HERE / "v6_results.json").write_text(json.dumps(out, indent=2), encoding="utf-8")
print(f"\nResults -> {HERE / 'v6_results.json'}")
