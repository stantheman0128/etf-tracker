"""Strategy A (fixed proxy): Real market-cap Top X on PIT SP500.

Cap proxy = today's marketCap × (price_at_date / price_today)
- For tickers Yahoo no longer covers (defunct), cap=0 → excluded from rank.
- This causes the cap-rank universe to effectively equal "today's still-tradable PIT members"
  but PIT timing is preserved (membership_at(date) still filters by SP500 membership at that date).
"""
...
# Sanity: how many tickers have valid today's cap?
n_with_cap = sum(1 for orig in USABLE if MKT_CAPS.get(STATUS[orig]["yahoo_sym"], {}).get("cap", 0) > 0)
print(f"Tickers with today's mkt cap: {n_with_cap} / {len(USABLE)} ({n_with_cap*100/len(USABLE):.1f}%)")

# Cache today's price per ticker for cap proxy denominator
TODAY_PRICE = {}
for orig in USABLE:
...
def cap_proxy(t, idx):
    """Historical cap = today_cap × (price_at_idx / today_price)."""
    ysym = STATUS[t]["yahoo_sym"]
    info = MKT_CAPS.get(ysym, {})
    today_cap = info.get("cap", 0) or 0
...
def build_strategy(n, rebal_days, select_random=False, seed=None):
...
print("\n" + "=" * 80)
print("STRATEGY A (FIXED): Real market-cap Top X on PIT SP500")
print("=" * 80)

t0 = time.time()
canonical = build_strategy(3, 21)
c_stats = stats(canonical)
print(f"\nCanonical (Top 3, monthly): CAGR {c_stats[0]*100:.1f}% | Sharpe {c_stats[3]:.2f} | MaxDD {c_stats[2]*100:.1f}% ({time.time()-t0:.1f}s)")

# Sanity: show top 3 at a few snapshot dates
print("\nSanity check — Top 3 by cap at selected dates:")
for sample_d in ["2001-01-02", "2008-01-02", "2015-01-02", "2020-01-02", "2025-01-02"]:
    if sample_d not in date_to_idx: continue
    idx = date_to_idx[sample_d]
    pit = universe_at(sample_d)
    caps = [(t, cap_proxy(t, idx)) for t in pit]
    caps = [(t, c) for t, c in caps if c > 0]
    caps.sort(key=lambda x: -x[1])
    top3 = caps[:3]
    print(f"  {sample_d}: {', '.join(f'{t}(${c/1e9:.0f}B)' for t, c in top3)}")
...
BONF = [(1, 21, "N=1, mthly"),(3, 21, "N=3, mthly"),(5, 21, "N=5, mthly"),
        (3, 63, "N=3, qtrly"),(1, 63, "N=1, qtrly"),(5, 63, "N=5, qtrly")]
PERMS = int(os.environ.get("V4B_PERMS", "1000"))
K = len(BONF); ALPHA = 0.05/K
bonf_results = []
total_t0 = time.time()
for vi, (n, rb, label) in enumerate(BONF):
    t0 = time.time()
    actual = build_strategy(n, rb)
    a_cagr = stats(actual)[0]
    perms = [stats(build_strategy(n, rb, select_random=True, seed=120000+vi*PERMS+s))[0] for s in range(PERMS)]
    perms.sort()
    n_beat = sum(1 for c in perms if c >= a_cagr)
    p_val = (n_beat + 1) / (PERMS + 1)
    survives = p_val < ALPHA
    print(f"  {label:<14} actual={a_cagr*100:>5.1f}% rand_med={perms[PERMS//2]*100:>5.1f}% p={p_val:.4f} {'YES' if survives else 'no':<3} ({time.time()-t0:.0f}s)")
...
print("\n" + "=" * 80)
print("STRATEGY A (FIXED) FINAL SCORECARD")
print("=" * 80)
...
out = {
    "strategy": "A_real_market_cap_top_x_pit",
...
(HERE / "v4b_results.json").write_text(json.dumps(out, indent=2), encoding="utf-8")
print(f"\nResults -> {HERE / 'v4b_results.json'}")
